...
- **Routing**: `expo-router`, file-based under `src/app/`. There's no top-level `app/` dir — expo-router falls back to `src/app/`. `typedRoutes` is on, so links/params are type-checked.
  - Two route groups gated by the root `_layout.tsx` via `<Stack.Protected guard={...}>`: `(app)` (authenticated; a Stack of `index` dashboard, `generator`, `exam`, `student`, `settings`) and `(auth)` (public; `sign-in`, `sign-up`, `verify-email`, `forgot-password`, `reset-password`, plus an `index` that redirects to `/sign-in`).
  - Both `(app)/index` and `(auth)/index` map to `/`; the guard decides which is live. This is the canonical expo-router auth pattern — keep it.
